...
for i in $list1;  
do  
 if [ -d "/root/pixiv/$i" ]; then
  gclone move /root/pixiv/$i gc:{0APhrGdH9CrpqUk9PVA}/排行榜/$i/$cur_y/$cur_m/$cur_d --ignore-existing
 fi
done 
list2="周榜 原创榜"  
for i in $list2;  
do  
 if [ -d "/root/pixiv/$i" ]; then
  gclone move /root/pixiv/$i gc:{0APhrGdH9CrpqUk9PVA}/排行榜/$i/$cur_y/${sm}${sd}-${cur_m}${cur_d} --ignore-existing
 fi
done 
list3="月榜"  
for i in $list3;  
do  
 if [ -d "/root/pixiv/$i" ]; then
  gclone move /root/pixiv/$i gc:{0APhrGdH9CrpqUk9PVA}/排行榜/$i/$cur_y/$cur_m --ignore-existing
 fi
done 
